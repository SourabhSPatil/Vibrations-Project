...
function_coeff = [C1*(sin(pi*x/2*l)) C2*(sin(3*pi*x/2*l)) C3*(sin(5*pi*x/2*l))];
function_mat = [(sin(pi*x/2*l)) (sin(3*pi*x/2*l)) (sin(5*pi*x/2*l))];
constant = [C1 C2 C3];
for k = 1:3
...
figure(1)
for i = 1:5
Y = mode_shape_coeff_single(1,i)*(sin(pi*X/2*l))+ mode_shape_coeff_single(2,i)*(sin(3*pi*X/2*l))+ mode_shape_coeff_single(3,i)*(sin(5*pi*X/2*l));
plot(X,Y)
hold on
end
title("First Mode")
...
figure(2)
for i = 1:3
Y = mode_shape_coeff_double(1,i)*(sin(pi*X/2*l))+ mode_shape_coeff_double(2,i)*(sin(3*pi*X/2*l))+ mode_shape_coeff_double(3,i)*(sin(5*pi*X/2*l));
plot(X,Y)
hold on
end
title("Second Mode")
...
figure(3)
for i = 3:5
Y = mode_shape_coeff_triple(1,i)*(sin(pi*X/2*l))+ mode_shape_coeff_triple(2,i)*(sin(3*pi*X/2*l))+ mode_shape_coeff_triple(3,i)*(sin(5*pi*X/2*l));
plot(X,Y)
hold on
end
title("Third Mode")
...
ylabel("response")
Legend = cell(3,1);
Legend{1} = num2str(Cb(3));
Legend{2} = num2str(Cb(4));
Legend{3} = num2str(Cb(5));
legend(Legend);
