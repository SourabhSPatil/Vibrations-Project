...
function_coeff = [C1*(sin(pi*x/l)) C2*(sin(2*pi*x/l)) C3*(sin(3*pi*x/l))];
function_mat = [(sin(pi*x/l)) (sin(2*pi*x/l)) (sin(3*pi*x/l))];
constant = [C1 C2 C3];
for k = 1:3
...
figure(1)
for i = 1:5
Y = mode_shape_coeff_single(1,i)*(sin(pi*X/l))+ mode_shape_coeff_single(2,i)*(sin(2*pi*X/l))+ mode_shape_coeff_single(3,i)*(sin(3*pi*X/l));
plot(X,Y)
hold on
end
title("First Mode")
...
figure(2)
for i = 1:4
Y = mode_shape_coeff_double(1,i)*(sin(pi*X/l))+ mode_shape_coeff_double(2,i)*(sin(2*pi*X/l))+ mode_shape_coeff_double(3,i)*(sin(3*pi*X/l));
plot(X,Y)
hold on
end
title("Second Mode")
...
figure(3)
for i = 1:5
Y = mode_shape_coeff_triple(1,i)*(sin(pi*X/l))+ mode_shape_coeff_triple(2,i)*(sin(2*pi*X/l))+ mode_shape_coeff_triple(3,i)*(sin(3*pi*X/l));
plot(X,Y)
hold on
end
title("Third Mode")
